...
using namespace std;
PonyhofP::PonyhofP() {
    ifstream infile("ponyss.txt"); // Öffnen der Datei "ponys.txt"
    if (!infile.is_open()) {
...
void PonyhofP::ponyHolen(){
    if(stallung.belgeteBoxen()>0){
        std::string name;
        std::cout<<"Name des zu holenden Ponys eingeben: ";
        std::cin>>name;
        PonyP* tmpr = stallung.herausholen(name);

        if(tmpr != NULL){
            unsigned art  = tmpr->getArt();
            if(art == 1){
                beimReiten.push_back(new ShetlandponyP(*dynamic_cast<ShetlandponyP*>(tmpr)));

                std::cout<<"Pony wird geritten\n";
            }
            else if (art == 0) {
                beimReiten.push_back(new IslandpferdP(*dynamic_cast<IslandpferdP*>(tmpr)));
